...
#
##############################################################################################
#' MKLE: Minimum Kullback-Leibler Divergence Estimate based on pdf for the 4-parameter GG4 distribution
#'
##
#
#' @description Fit of univariate distributions to non-censored data by minimum Kullback-Leibler divergence based on pdf.
#'
##
#
  ...
  if(all(y<0)) {
    ...
    y = -y}
        ...
        distkl <- function(theta, y){
          ...
          #print(sum(xx))
          #print(sum(yy))
          #if (a>0) print(log(yy/xx))
          #fct <- sum(yy*log(yy/xx))
          fct <- sum(abs(yy*log(yy/xx)))
          #fct <- sum(xx*log(xx/yy))
          return(fct)
        }
