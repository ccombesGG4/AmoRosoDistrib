...
#
#############################################################################
###' Truncated probability density function of
###' four-parameter Generalized Gamma Distribution
###
#' @param q vector of quantile
#' @param trt vector of quantile for truncated pdf
...
truncdgg4 <- function(q, a, l, c, mu = 0, trt,  log.p = FALSE)
{
  ...
  #tggampdf <- function(x, alpha, eta, kappa, trt, log.p=F) {
    Y <- dgg4(sort(q), a, l, c, mu,  log.p = FALSE) - pgg4(sort(trt), a, l, c, mu, lower.tail = FALSE, log.p = TRUE)
    if (log.p) Y <- log(Y)
    return (Y)
}
